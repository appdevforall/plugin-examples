...
internal class CredentialFailureLog(private val prefs: () -> SharedPreferences?) {
     ...
     */
    fun record(failure: OpenAiFailure, keyStamp: Long) {
        val credential = CredentialFailure.of(failure) ?: return
        prefs()?.edit()
            ?.putString(OpenAiPreferences.KEY_CREDENTIAL_FAILURE, credential.tag)
            ?.putLong(OpenAiPreferences.KEY_CREDENTIAL_FAILURE_KEY_STAMP, keyStamp)
            ?.apply()
    }
    ...
    fun read(): CredentialFailure? {
        ...
        val tag = prefs.getString(OpenAiPreferences.KEY_CREDENTIAL_FAILURE, null)
