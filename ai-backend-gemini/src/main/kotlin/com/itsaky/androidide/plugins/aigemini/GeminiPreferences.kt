...
internal object GeminiPreferences {
    ...
    /**
     * Plugin ids whose shared `AgentSettings` may still hold this backend's values, newest first.
     *
     * Both are searched because plugins load in parallel with no ordering: AI Core adopts the older
     ...
     * take.
     */
    private val LEGACY_PLUGIN_IDS = listOf(
        "com.itsaky.androidide.plugins.aicore",
        "com.itsaky.androidide.plugins.aiassistant",
    )
    ...
    /**
     * Copies this backend's settings out of the agent plugin's shared file, once.
     *
    ...
    fun migrateIfNeeded(context: PluginContext) {
        ...
        try {
            for (pluginId in LEGACY_PLUGIN_IDS) {
                val legacy = context.getAppSharedPreferences(legacyFileName(pluginId)) ?: continue
                if (copyOwnedValues(legacy, prefs)) {
                    context.logger.info("GeminiPreferences: adopted settings from $pluginId")
                    break
                }
            }
