...
This guide explains how to build the AI assistant plugins. It lives here because
`ai-backend-local` is the only one with a native build; `ai-core` and
`ai-backend-gemini` are plain Kotlin plugins that need nothing beyond the SDK.

> **TL;DR** — A normal build needs only the Android SDK + JDK 17. The native
> llama.cpp library ships **prebuilt** as `ai-backend-local/libs/v8/llama-v8-release.aar`,
...
(`ai-backend-local/`, `ai-core/`, `ai-backend-gemini/`, and, for the frontend,
`ai-assistant/`):
...
already committed as `ai-backend-local/libs/v8/llama-v8-release.aar` (the JNI
wrapper plus `.so` files for arm64-v8a, armeabi-v7a, x86 and x86_64), with its
tiny interface jar at `ai-backend-local/libs/llama-api.jar`.
...
directory. `ai-backend-local` uses the shared wrapper at the repo root, so it is
invoked as `../gradlew`:

```bash
# release .cgp for each plugin
cd plugin-examples/ai-backend-local  && ../gradlew assemblePlugin
cd ../ai-backend-gemini              && ../gradlew assemblePlugin
cd ../ai-core                        && ./gradlew assemblePlugin
cd ../ai-assistant                   && ./gradlew assemblePlugin

# Debug variant
cd plugin-examples/ai-backend-local  && ../gradlew assemblePluginDebug
```

### Option B: Build via Android Studio

1. Open Android Studio
2. **File → Open** → Select `plugin-examples/ai-backend-local/` (or `plugin-examples/ai-assistant/`)
...
```
ai-backend-local/build/plugin/ai-backend-local.cgp
...
adb push ai-backend-local/build/plugin/ai-backend-local.cgp /sdcard/Download/
...
# 4. Select ai-backend-local.cgp first
...
```bash
# Run from the ai-backend-local plugin dir. One command: inits the submodule, compiles
# :llama-impl + :llama-api from source, copies fresh artifacts into ai-backend-local/libs/.
cd plugin-examples/ai-backend-local
...
Then commit the refreshed `ai-backend-local/libs/v8/llama-v8-release.aar` and
`ai-backend-local/libs/llama-api.jar`. The `:llama-impl` / `:llama-api` Gradle
...
```bash
unzip -l ai-backend-local/libs/v8/llama-v8-release.aar | grep 'jni/'
```
...
```bash
# Clean this plugin (run from ai-backend-local/)
../gradlew clean
...
4. **ai-backend-local** uses the llama-api/llama-impl modules to provide LLM inference services
5. **ai-core** routes requests to it, and **ai-assistant** consumes ai-core via SharedServices
...
```
ai-assistant → (SharedServices) → ai-core → (registered backend) → ai-backend-local
                                                                        ↘ libs/v8/llama-v8-release.aar (native)
...
llama-api  → interfaces
   ⇒ scripts/rebuild-llama-aar.sh copies their output into ai-backend-local/libs/
```
...
ai-backend-local/build/outputs/apk/v8/release/ai-backend-local-v8-release.apk
...
jarsigner -verbose -sigalg SHA256withRSA -digestalg SHA-256 \
  ...
  ai-backend-local-v8-release.apk ai-plugins
```

### Size Optimization

Current release sizes:
- **ai-backend-local**: ~5 MB (includes llama.cpp native libraries)
...
2. Verify **ai-core**, **ai-backend-local**, **ai-backend-gemini** and **ai-assistant** work together
