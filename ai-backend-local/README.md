# AI Local Backend plugin for CodeOnTheGo
...
```bash
cd ai-backend-local
../gradlew assemblePlugin          # release  -> build/plugin/ai-backend-local.cgp
...
- `LocalLlmPlugin.kt` — plugin entry point; registers the backend with ai-core
- `LocalLlmBackend.kt` — on-device GGUF inference over the llama.cpp AAR
- `GgufModelInspector.kt` — refuses embedding-only models before they SIGABRT
- `ModelLoadDiagnostics.kt` / `ModelLoadMessages.kt` — load-failure classification
  and its user-facing wording
