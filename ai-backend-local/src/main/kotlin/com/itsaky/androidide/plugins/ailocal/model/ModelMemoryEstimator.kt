...
import com.itsaky.androidide.plugins.ailocal.model.GgufHeader
 ...
 * Android-free, so the arithmetic is unit-testable. The context and batch sizes below are ai-backend-local's,
 * fixed on its native side: an estimate has to model the loader that will actually run.
 */
object ModelMemoryEstimator {

    /**
     * The context every load gets, hard-coded as `ctx_params.n_ctx` in ai-backend-local's `llama-android.cpp`.
    ...
    /**
     * Graph and compute buffers for the 2048-token batch ai-backend-local allocates (`new_batch` in
     ...
     * ai-backend-local so this warning and the refusal that gates the load itself cannot
