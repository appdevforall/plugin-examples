...
class RunAppHandler(
    ...
    override suspend fun execute(args: Map<String, Any?>): ToolResult {
        return try {
            Log.d(TAG, "Run app tool called - v3 with retry")
            ...
            Log.d(TAG, "BuildService obtained successfully")

            val buildInProgress = buildService.isBuildInProgress()
            Log.d(TAG, "Build in progress: $buildInProgress")
            ...
            for (attempt in 1..maxRetries) {
                ...
                Log.d(TAG, "Tooling server check (attempt $attempt/$maxRetries): $toolingStarted")

                if (toolingStarted) {
                    Log.d(TAG, "Tooling server is now ready after $totalWaitTime ms")
                    break
                }

                if (attempt < maxRetries) {
                    val delayMs = 300L * attempt // 300ms, 600ms, 900ms, 1.2s, 1.5s, 1.8s, 2.1s, 2.4s, 2.7s
                    Log.d(TAG, "Tooling server not ready, waiting ${delayMs}ms before retry...")
                    delay(delayMs)
            ...
            if (!toolingStarted) {
                Log.w(TAG, "Tooling server did not initialize within ${totalWaitTime + 300}ms, proceeding anyway (may fail)")
                // Try to proceed anyway - the service might initialize during the build
                Log.d(TAG, "Attempting to run app despite tooling server not being ready...")
            }
            ...
            return try {
                ...
                buildService.runApp(object : BuildAndLaunchCallback {
                    override fun onComplete(success: Boolean, message: String) {
                        Log.i(TAG, "Build callback: success=$success, message=$message")
                    }
                })

                Log.d(TAG, "Build triggered, returning success")
                ToolResult.success(
                    message = "Build triggered successfully",
                    data = "The app build is now running in the background. Output will appear in the IDE's build panel."
                )
            } catch (e: Exception) {
                Log.e(TAG, "Failed to trigger build: ${e.message}", e)
                ToolResult.failure(
