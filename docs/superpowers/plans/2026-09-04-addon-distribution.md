...
- [ ] **Step 4: Run the workflow with `staging: true`** — **DEFERRED by the owner.** Closed out at 98 of 99 rather than merging early. Blocked until this branch merges: `workflow_dispatch` only triggers a workflow that exists on the default branch, so `gh workflow run` returns 404 while `publish-addons.yml` lives on a feature branch. Nothing else is missing: the secrets are configured and the workflow maps them correctly.
