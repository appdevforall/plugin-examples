...
import org.appdevforall.codeonthego.layouteditor.EditorSubScreenState
import org.appdevforall.codeonthego.layouteditor.PreviewLayoutFragment
import org.appdevforall.codeonthego.layouteditor.openLayoutEditorScreen
import org.appdevforall.codeonthego.layouteditor.databinding.ListProjectFileBinding
...
class ProjectListAdapter(private val projects: MutableList<ProjectFile>) :
  ...
  private fun checkNameErrors(
    ...
    for (file in projects) {
      ...
      if (file.name == name) {
        inputLayout.isErrorEnabled = true
        inputLayout.error = inputLayout.context.getString(string.msg_current_name_unavailable)
        dialog.getButton(AlertDialog.BUTTON_POSITIVE).isEnabled = false
        return
      }
    }
    ...
    scope.launch {
      try {
        withContext(Dispatchers.IO) {
          ...
          val projectDir =
            "${FileUtil.getPackageDataDir(v.context)}/projects/${projects[position].name}"
        ...
        Toast.makeText(v.context, v.context.getString(string.msg_feature_not_available), Toast.LENGTH_SHORT).show()
			} catch (e: Exception) {
    ...
    MaterialAlertDialogBuilder(v.context)
      ...
      .setAdapter(
        ...
        if (allLayouts[w].readDesignFile().isEmpty()) {
          ...
          EditorSubScreenState.previewLayoutFile = allLayouts[w]
          openLayoutEditorScreen(PreviewLayoutFragment::class.java.name, "")
        }
        d.dismiss()
      }
