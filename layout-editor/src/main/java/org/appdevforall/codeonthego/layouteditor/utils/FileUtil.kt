...
object FileUtil {
  ...
  fun readFromAsset(path: String?, ctx: Context): String {
    try {
      // Assets are bundled inside the .cgp, so read them from the plugin's own context
      // (the passed-in host context has no plugin assets). Falls back to ctx when not running
      // as a plugin.
      val assetSource = PluginFragmentHelper.getPluginContext(LayoutEditorPlugin.PLUGIN_ID) ?: ctx
      val inputStream = assetSource.assets.open(path!!)
  ...
  fun copyFileFromAsset(filename: String, outPath: String) {
    // Assets live in the plugin (.cgp), so read them from the plugin's own context.
    val assetManager = (PluginFragmentHelper.getPluginContext(LayoutEditorPlugin.PLUGIN_ID)
      ?: error("Layout Editor plugin context unavailable for asset access")).assets
