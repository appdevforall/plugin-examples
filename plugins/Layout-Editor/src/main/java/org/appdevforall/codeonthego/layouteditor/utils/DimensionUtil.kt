  ...
  fun parse(input: String, contxt: Context): Float {
    when (input) {
      ...
      "wrap_content" -> return ViewGroup.LayoutParams.WRAP_CONTENT.toFloat()
        ...
        val numberString =
          if (dimen != null) input.substring(0, input.lastIndexOf(dimen)) else input
        val number = numberString.toFloatOrNull() ?: run {
          Log.w("DimensionUtil", "Unparseable dimension '$input'; defaulting to wrap_content")
          return ViewGroup.LayoutParams.WRAP_CONTENT.toFloat()
        }

        // Returning calculated dimension value
        return TypedValue.applyDimension(
          dimensMap[dimen] ?: TypedValue.COMPLEX_UNIT_DIP, number, contxt.resources.displayMetrics
        )
      }
    }
  }
